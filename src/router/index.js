...
import { getToken, canTurnTo, setTitle, getUserIdFromToken } from "@/libs/util";
...
router.beforeEach((to, from, next) => {
  ...
  if (token) {
    store.state.user.userId = getUserIdFromToken(token);
  }
  if (!token && to.name !== LOGIN_PAGE_NAME) {
    ...
    if (to.path !== '/edge_management' && store.state.edgeManagement.edgeList.length <= 0) {
      next({
